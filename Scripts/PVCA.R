...
PlotPVCA <- function(pvca.res, title){
  ...
  p <- p + geom_text(aes(label=round(prop,3), y=prop+0.04), size=6)
  ...
  p <- p + theme(axis.title.x = element_text(size = 18, vjust=-0.5))
  p <- p + theme(axis.title.y = element_text(size = 18, vjust= 1.0))
